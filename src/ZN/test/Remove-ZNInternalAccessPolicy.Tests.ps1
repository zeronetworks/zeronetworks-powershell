if(($null -eq $TestName) -or ($TestName -contains 'Remove-ZNInternalAccessPolicy'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Remove-ZNInternalAccessPolicy.Recording.json'
